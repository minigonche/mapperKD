...
#' @author Felipe González-Casabianc
#' mapperkD function
#'
...
mapperKD <- function(
  ...
  #- for (level in 1:num_levels)
   repeat {
    ...
    last = any(filter_min + ((level_indices + 1) * step_size) + interval_length > filter_max)
    # gonche: bug de sensibilidad numerica. EL ultimo intervalo a veces no llega al maximo
    if(last)
    {
      print("Last Interval")
      max_value_in_level = filter_max
    }
    ...
    #print(which(points_in_level_logical))
