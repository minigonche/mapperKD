...
# get_1_skeleton_node_sizes
#' Gets the size of each of the 1 skeleton nodes. The size of each node is the amount of point it cointains
#' @param one_skeleton_result A one_skeleton object to calculate the sizes of the nodes
#' @return A vector with the corresponding sizes
get_1_skeleton_node_sizes = function(one_skeleton_result)
{
  # Calculates the size
  return(sapply(one_skeleton_result$points_in_nodes, length))
}
...
#' Constructs a grid layout of the 1 skeleton (graph). If filter has only one dimension, the vertical layout is assumed evenly
#' If filter has more than two dimensional an exception is raised
#' @param one_skeleton_result A one_skeleton object to calculate the grid layout
#' @return A matrix n x 2, where n is the number of nodes in the 1 skeleton
construct_grid_graph_layout = function(one_skeleton_result)
{
  ...
  if(filter_dim > 2)
    stop('The received 1 skeleton object was constructed using a filter with more than two dimensions. Maximum of two is allowed.')

  # Creates the return variables
  x = rep(0, one_skeleton_result$num_nodes)
  y = rep(0, one_skeleton_result$num_nodes)

  max_dimension = length(one_skeleton_result$nodes_per_interval)
  ...
  for(i in 1:max_dimension)
  {
    ...
    if(filter_dim == 1) # One dimensional case
    {
      ...
      nodes_in_interval = one_skeleton_result$nodes_per_interval[[i]]
    ...
    {
      for(j in 1:length(one_skeleton_result$nodes_per_interval[[i]]))
      {
        # Nodes inside the interval
        nodes_in_interval = one_skeleton_result$nodes_per_interval[[c(i,j)]]
...
#' Calculates the filter's dimension of the provided 1 skeleton
#' @param one_skeleton_result A one_skeleton object to calculate the filter's dimension
#' @return An int with the corresponding dimension
get_filter_dimension = function(one_skeleton_result)
{
  n = 1
  while(TRUE)
  {
    if(!is.list(one_skeleton_result$nodes_per_interval[[rep(1,n)]]))
    {
      return(n)
    }
    n = n + 1
...
# plot_1_skeleton
#' Plots the given 1 skeleton. The sizes of the nodes are proportional to the amount of elements they group.
#' @param one_skeleton_result A one_skeleton object to plot
...
{
  # Computes centrality
  centrality = as.vector(table(unlist(one_skeleton_result$points_in_nodes)))
  return(centrality)
}
...
create_point_intersection_adjacency = function(one_skeleton_result)
{
  ...
  # Sorry for the for
  for(i in 1:one_skeleton_result$num_nodes)
  {
...
create_point_intersection_network = function(one_skeleton_result)
{
  # Loads igraph
  require('igraph')
...
plot_intersection_network = function(one_skeleton_result, groups = NULL, min_node_size = 3, max_node_size = 20)
{
  # Loads igraph
  require('igraph')

  # Creates the network
  pin = create_point_intersection_network(one_skeleton_result)
                                              ...
                                              black_white = TRUE)
{
  ...
  elements = create_plot_elements_for_point_intersection_network(one_skeleton_result, lon = final_lon, lat = final_lat)
  ...
  # Extracts the maps background color
  back_color = 'bw'
  if(!black_white)
    back_color = 'color'
...
#'   - Arrows (DataFrame): where x1, x2 are the longitude of the start and finish of the arrows, y1,y2 the latitudes and origin, destination the corresponding nodes for each arrow.
create_plot_elements_for_point_intersection_network = function(one_skeleton_result, lon, lat)
{
  ...
  response = list()
  response[['Points']] = data.frame(lon = lon, lat = lat)
  response[['Arrows']] = data.frame(x1 = x1, y1 = y1, x2 = x2, y2 = y2, origin = org, destination = dest)
...
# hamming_distance
#' Funtion that calculates the hamming distance between the point intersection networks of two one skeleton elements. If the parameters don't have the same number of points,
#' an error will be raised.
#' @param one_skeleton_1 The first result
#' @param one_skeleton_2 The second result
...
# extract_color_legend
#' Support function for extracting the color legend that ggplot will assign to each group element
#' @param groups Vector with the aestetic vector that ggplot will use
#' @return List with the following elements:
#'    - Labels: vector if strings with the labels of the legend
#'    - Colors: vector with the colors of the legend
extract_color_legend = function(groups)
{
